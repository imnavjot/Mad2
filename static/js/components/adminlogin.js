...
const AdminLogin = Vue.component("adminlogin", {
    ...
    <div>
      ...
      <div class="container d-flex justify-content-center align-items-center vh-100">
        <div class="card p-4">
          ...
          <p class="mt-3">Store Manager? <router-link to="/store_manager/login">SM Login</router-link></p>
          <p class="mb-0">User? <router-link to="/user/login">User Login</router-link></p>
        </div>
      </div>
    </div>
  `,
  ...
  methods: {
    admin_login() {
      // fetch API request to perform admin login with this.email and this.password
      fetch('/admin/login', {
        method: 'POST',
        headers: {
